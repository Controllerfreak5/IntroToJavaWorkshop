...
public class TurtleTreasureHunt implements KeyEventDispatcher {
	...
	private void spaceBarWasPressed() {
		...
		if (tortoiseLocationX >= 500 && tortoiseLocationX <= 510 && tortoiseLocationY >= 290 && tortoiseLocationY <= 340) {
			JOptionPane.showMessageDialog(null, "Go see the boy for your next clue.");
		talkedto = 1;
		
		}
		if (tortoiseLocationX >= 140 && tortoiseLocationX <= 150 && tortoiseLocationY >= 280 && tortoiseLocationY <= 330 && talkedto == 1)
			
		{
			JOptionPane.showMessageDialog(null, "The pirate should be able to help you on your quest.");
		talkedto = 2;
		}

		//if (tortoiseLocationX >=)
